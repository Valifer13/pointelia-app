...
<div class="bg-white w-full overflow-x-auto p-4 rounded-md shadow-md mb-6">
    <table class="w-full text-left border-collapse">
        ...
        <tbody class="text-sm">
            ...
            <?php
            $count = ($student_classes['pagination']['current_page'] - 1) * 10 + 1;
            foreach ($student_classes['data'] as $student_class):
            ?>
...
<div class="pagination flex gap-5 py-4 justify-center px-5 bg-white rounded-md shadow-md">
    <ul class="flex justify-center gap-3 text-gray-900">
        <li>
            <a href="<?= $student_classes['pagination']['has_prev'] ? BASE_URL . '/classes/page/' . ($student_classes['pagination']['current_page'] - 1) : '#' ?>" class="grid size-8 place-content-center rounded border border-gray-200 transition-colors hover:bg-gray-50 rtl:rotate-180" aria-label="Previous page">
        ...
        <li class="text-sm/8 font-medium tracking-widest"><?= $student_classes['pagination']['current_page'] ?>/<?= $student_classes['pagination']['last_page'] ?></li>

        <li>
            <a href="<?= $student_classes['pagination']['has_next'] ? BASE_URL . '/classes/page/' . ($student_classes['pagination']['current_page'] + 1) : '#' ?>" class="grid size-8 place-content-center rounded border border-gray-200 transition-colors hover:bg-gray-50 rtl:rotate-180" aria-label="Next page">
                <svg xmlns="http://www.w3.org/2000/svg" class="size-4" viewBox="0 0 20 20" fill="currentColor">

...
<div class="pagination flex gap-5 py-4 justify-center px-5 bg-white rounded-md shadow-md">
    <ul class="flex justify-center gap-1 text-gray-900">
        ...
        <!-- PREVIOUS -->
        <li>
            <a href="<?= $guardians['pagination']['has_prev'] ? BASE_URL . '/guardians/page/' . ($guardians['pagination']['current_page'] - 1) : '#' ?>"
               class="grid size-8 place-content-center rounded border border-gray-200 transition-colors 
               <?= !$guardians['pagination']['has_prev'] ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50' ?>"
        ...
        <!-- PAGE NUMBERS -->
        <?php for ($i = 1; $i <= $guardians['pagination']['last_page']; $i++): ?>
            <li>
                <?php if ($i == $guardians['pagination']['current_page']): ?>
                    <span class="block size-8 rounded border border-blue-600 bg-blue-600 text-center text-sm/8 font-medium text-white">
                        <?= $i ?>
                    </span>
                <?php else: ?>
                    <a href="<?= BASE_URL ?>/guardians/page/<?= $i ?>"
        ...
        <!-- NEXT -->
        <li>
            <a href="<?= $guardians['pagination']['has_next'] ? BASE_URL . '/guardians/page/' . ($guardians['pagination']['current_page'] + 1) : '#' ?>"
               class="grid size-8 place-content-center rounded border border-gray-200 transition-colors 
               <?= !$guardians['pagination']['has_next'] ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50' ?>"

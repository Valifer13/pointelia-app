<div class="flex flex-col md:flex-row justify-between gap-4 items-start lg:items-center mb-6 bg-white p-4 rounded-md shadow-md">
    ...
    <div class="flex gap-4">
        ...
        <a href="<?= BASE_URL ?>/guardians/add" class="flex gap-2 items-center px-3 py-2 bg-blue-600 hover:bg-blue-700 transition-colors duration-300 cursor-pointer text-white rounded-md font-medium">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
...
<div class="bg-white w-full overflow-x-auto p-4 rounded-md shadow-md mb-6">
    <table class="w-full text-left border-collapse">
        ...
        <tbody class="text-sm">
            ...
            <?php
            $count = ($guardians['pagination']['current_page'] - 1) * 10 + 1;
            foreach ($guardians['data'] as $guardian):
            ?>
...
<div class="pagination flex gap-5 py-4 justify-center px-5 bg-white rounded-md shadow-md">
    <ul class="flex justify-center gap-3 text-gray-900">
        <li>
            <a href="<?= $guardians['pagination']['has_prev'] ? BASE_URL . '/guardians/page/' . ($guardians['pagination']['current_page'] - 1) : '#' ?>" class="grid size-8 place-content-center rounded border border-gray-200 transition-colors hover:bg-gray-50 rtl:rotate-180" aria-label="Previous page">
        ...
        <li class="text-sm/8 font-medium tracking-widest"><?= $guardians['pagination']['current_page'] ?>/<?= $guardians['pagination']['last_page'] ?></li>

        <li>
            <a href="<?= $guardians['pagination']['has_next'] ? BASE_URL . '/guardians/page/' . ($guardians['pagination']['current_page'] + 1) : '#' ?>" class="grid size-8 place-content-center rounded border border-gray-200 transition-colors hover:bg-gray-50 rtl:rotate-180" aria-label="Next page">
                <svg xmlns="http://www.w3.org/2000/svg" class="size-4" viewBox="0 0 20 20" fill="currentColor">

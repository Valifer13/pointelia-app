<div class="flex flex-col md:flex-row justify-between gap-4 items-start md:items-center mb-6 bg-white p-4 rounded-md shadow-md">
    <div class="flex flex-col gap-1 w-full">
        <h1 class="text-2xl font-semibold tracking-wide text-zinc-600">List Tipe Pelanggaran</h1>
        <?php get_breadcrumb() ?>
    </div>
    <div class="flex gap-4 flex-col md:flex-row w-full md:w-fit items-center">
        <button class="flex gap-2 items-center px-3 py-2 bg-white border border-zinc-300 text-zinc-800 rounded-md cursor-pointer transition-colors duration-300 justify-center md:justify-start w-full md:w-fit">
        ...
        <a href="<?= BASE_URL ?>/violation-types/add" class="flex gap-2 items-center px-3 py-2 bg-blue-600 hover:bg-blue-700 transition-colors duration-300 cursor-pointer text-white rounded-md font-medium justify-center md:justify-start w-full md:w-fit whitespace-nowrap">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
...
<div class="pagination flex gap-5 py-4 justify-center px-5 bg-white rounded-md shadow-md">
    <ul class="flex justify-center gap-3 text-gray-900">
        <li>
            <a href="<?= $violation_types['pagination']['has_prev'] ? BASE_URL . '/violations-types/page/' . ($violation_types['pagination']['current_page'] - 1) : '#' ?>" class="grid size-8 place-content-center rounded border border-gray-200 transition-colors hover:bg-gray-50 rtl:rotate-180" aria-label="Previous page">
        ...
        <li class="text-sm/8 font-medium tracking-widest"><?= $violation_types['pagination']['current_page'] ?>/<?= $violation_types['pagination']['last_page'] ?></li>

        <li>
            <a href="<?= $violation_types['pagination']['has_next'] ? BASE_URL . '/violations-types/page/' . ($violation_types['pagination']['current_page'] + 1) : '#' ?>" class="grid size-8 place-content-center rounded border border-gray-200 transition-colors hover:bg-gray-50 rtl:rotate-180" aria-label="Next page">
                <svg xmlns="http://www.w3.org/2000/svg" class="size-4" viewBox="0 0 20 20" fill="currentColor">
